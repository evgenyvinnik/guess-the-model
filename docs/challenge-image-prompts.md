...
132 playable originals across 15 difficult prompts are saved and registered, with at least four different providers for every prompt. Provider totals are ChatGPT 24, FLUX 24, Meta AI 24, Gemini 24, Copilot 15, native Firefly 12, Yandex Alice 6, Qwen 2, and Grok Imagine 1. The full manifest contains 146 images, including fourteen archived baseline outputs. The target is 24 per provider where each service permits it. Only saved files with manifest entries count toward these totals.
...
Numbers are playable saved outputs per provider; a dash means no playable output. Beyond the six columns, Grok Imagine has the bicycle; Yandex Alice has the bicycle, archer, shoelace, candles, ceramic chain, and piano; Qwen has the bicycle and archer.
...
The later archer attempt in Grok Imagine displayed an upgrade gate before submission, so it produced no new original. A further Qwen shoelace request prompted age confirmation before submission; no age was supplied and no output was counted. The age check remains with the user.
