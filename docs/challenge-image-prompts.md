...
Four-image rounds require four distinct providers with the same prompt id and exact submitted text. Single-image rounds use the same bank. Browser-local history shows every single-image original before repeating one, with provider balance used among models that still have unseen artwork. Comparison targets stay unseen while eligible artwork remains, and their four-image sets favor new originals, then older scenes and balanced provider exposure. All four displayed comparison images count as seen. The fourteen archived fox, robot, and teapot images and later duplicate variants remain available in a collapsed historical statistics section and are excluded from new rounds. Grok Imagine, Yandex Alice, and Qwen can appear in either format; sparse providers are not repeatedly selected merely to catch up to larger banks.
