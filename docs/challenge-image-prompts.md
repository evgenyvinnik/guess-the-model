...
129 playable originals across 15 difficult prompts are saved and registered, with at least four different providers for every prompt. Provider totals are ChatGPT 24, FLUX 24, Meta AI 24, Gemini 24, Copilot 15, native Firefly 12, Yandex Alice 3, Qwen 2, and Grok Imagine 1. The full manifest contains 143 images, including fourteen archived baseline outputs. The target is 24 per provider where each service permits it. Only saved files with manifest entries count toward these totals.
...
Numbers are playable saved outputs per provider; a dash means no playable output. Beyond the six columns, Grok Imagine has the bicycle; Yandex Alice has the bicycle, archer, and shoelace; Qwen has the bicycle and archer.
...
- **Yandex Alice:** three September 22 challenge originals (bicycle, archer, shoelace), all 1024 × 1024 JPEGs generated through Alice's Draw picture tool and downloaded through the signed-in Chrome UI. The tool prefixed each otherwise identical English challenge text with “нарисуй” (draw). The interface did not expose a numbered image-model version. The shoelace output has a solid red lace rather than the requested ivory lace with a red stripe; the original is retained unchanged.
- **Qwen:** two September 22 challenge originals (bicycle and archer), both 2048 × 2048 PNGs generated in Qwen Studio Create Image mode with Qwen-Image 3.0 explicitly selected and labeled on the output. The exact English challenge prompts were submitted unchanged. Chrome's Download navigation for the bicycle was blocked by the browser, so the full-resolution asset already rendered in the page was saved unchanged; the archer original was captured the same way. No preview or screenshot was used.
...
The later archer attempt in Grok Imagine displayed an upgrade gate before submission, so it produced no new original. A further Qwen shoelace request prompted age confirmation before submission; no age was supplied and no output was counted.
